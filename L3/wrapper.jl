    ...
    for sim in 1:case_nsim
        ...
        if use_diff == 1
            ...
            for ii in eachindex( diff_order )
                ...
                R_sim_diff_all[ii][ :, ind_start:ind_end ] = @view N_diffs[ii][ :, : ]
            end
        end
    end

    # println( size(R0_all) )
    # println( size(R_sim_all) )
    # println( size(R0_diff_all[1]) )
    # println( size(R_sim_diff_all[1]) )
    # println( R_sim_diff_all[1] )

    if use_diff == 1
        # R0_all = [ R0_all, R0_diff_all... ]  # Concatenate all R0 and R0_diff
