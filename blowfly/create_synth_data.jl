function create_synth_data( data::Dict{Symbol, Any} )
    ...
    for ii in 1:nepo
        ...
        N_burned_temp = @view N[ 1:1, Int(data[:params][5])+burn_in-1:end ]  # Remove the first lag and burn-in days
        R0_full[ii] = N
        R0[ii] = @view N_burned_temp[ 1:1, 2:end-1 ]
        if use_diff == 1
            R0_diff_ii = calculate_diffs( N_burned_temp, diff_order, dt )
